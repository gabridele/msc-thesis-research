#!/bin/bash
#script to check if output from 3ddeconvolve and 3dremlfit has significant changes (it shouldnt)
#files to correlate have to be in the same directory, corresponding brainmask doesn't have to.
...
perform_correlation() {
    ...
    echo "Non-dec file: $non_dec_file"
    echo "Using mask file: $mask_file"

    # Run 3ddot command to calculate correlation with mask file
    echo "for sub-$subject_id, $dec_file, $non_dec_file and mask file $mask_file:" >> "$output_file"
    echo "" >> "$output_file"
...
# Find all dec files and process them sequentially
