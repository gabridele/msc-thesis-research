...
function makeTimings {
	input="$1"
	output="${input%_task-scap_events.tsv}"
	
	if [ -f "$output" ]; then
        	echo "Output file $output already exists, skipping..."
	else
		echo "processing $input"
		cat "$input" | awk '{if ($3=="1" || $3=="2" || $3=="3" || $3=="4" || $3=="5" || $3=="6") {print $1, $2, 1}}' > "${output}_low_WM.txt"
		cat "$input" | awk '{if ($3=="7" || $3=="8" || $3=="9" || $3=="10" || $3=="11" || $3=="12") {print $1, $2, 1}}' > "${output}_high_WM.txt"
		echo "${output}_high_WM.txt"
	fi
}

export -f makeTimings
find "$path_raw" -type f -name '*sub-*task-scap_events.tsv' > "$path_raw/input_files.txt"
cat "$path_raw/input_files.txt" | parallel -j "$numjobs" makeTimings {}
rm "$path_raw/input_files.txt"
#----
#0.1 convert to AFNI
function convert_AFNI {
	input="$1"
	output="${input%txt}"
	...
	if [ -f "$output" ]; then
		...
		timing_tool.py -fsl_timing_files "$input" -write_timing "${output}1D"
	fi
}

export -f convert_AFNI
find "$path_raw" -type f -name '*_WM.txt' > "$path_raw/input_files.txt"
cat "$path_raw/input_files.txt" | parallel -j "$numjobs" convert_AFNI {}
rm "$path_raw/input_files.txt"
#-----
#1
function smooth { 
...
#3 make binarized image into same size as t1 brain mask
function resample {
    input="$1"
    output="${input%.nii.gz}_bin_resampled.nii.gz"
    mask="${input%_class-CSF_bin.nii.gz}_brainmask.nii.gz"  
...
find "$path_raw" -type f -name '*CSF_probtissue_bin.nii.gz' > "$path_raw/input_files.txt"
cat "$path_raw/input_files.txt" | parallel -j "$numjobs" resample {} "$mask"
rm "$path_raw/input_files.txt"

#4 make epi smoothed same size as t1 csf scan
function resample2 { 
    input="$1"
...
cat "$path_input/input_files.txt" | parallel -j "$numjobs" resample2 {} "$mask"
rm "$path_input/input_files.txt"
...
=======
find "$path_raw/input_files.txt" -type f -name '_preproc_smoothed_resampled.nii.gz' > "$path_input/input_files.txt"
>>>>>>> aea18fe3e736a3715c305f11d340cd29819816ef
...
=======
-input '/home/gabridele/Desktop/spreading_dynamics_clinical/derivatives/sub-10159/func/sub-10159_task-scap_bold_space-MNI152NLin2009cAsym_preproc_smoothed_resampled.nii.gz' \
-polort 'A' \
-num_stimts 10 \
-stim_times 1 '/home/gabridele/Desktop/spreading_dynamics_clinical/derivatives/sub-10159/func/sub-10159_low_WM.txt' 'GAM' -stim_label 1 low_WM \
-stim_times 2 '/home/gabridele/Desktop/spreading_dynamics_clinical/derivatives/sub-10159/func/sub-10159_high_WM.txt' 'GAM' -stim_label 2 high_WM.txt \
>>>>>>> aea18fe3e736a3715c305f11d340cd29819816ef
...
=======
-stim_file 9 '/home/gabridele/Desktop/spreading_dynamics_clinical/derivatives/sub-10159/func/sub-10159_task-scap_mean_timeseriesCSF.tsv''[0]' -stim_base 9 -stim_label 9 csf \
>>>>>>> aea18fe3e736a3715c305f11d340cd29819816ef
