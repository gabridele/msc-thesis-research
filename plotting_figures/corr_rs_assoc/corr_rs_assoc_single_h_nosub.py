...
def compute_spearman_correlation_h(csv_path, npy_path):
    ...
    lh_csv_values = np.nan_to_num(lh_csv_ctx[np.triu_indices_from(lh_csv_ctx, k=1)].flatten())
    rh_csv_values = np.nan_to_num(rh_csv_ctx[np.triu_indices_from(rh_csv_ctx, k=1)].flatten())

    lh_npy_ctx = upper_npy[54:254, 54:254]
    rh_npy_ctx = upper_npy[254:454, 254:454]
    lh_npy_values = np.nan_to_num(lh_npy_ctx[np.triu_indices_from(lh_npy_ctx, k=1)].flatten())
    rh_npy_values = np.nan_to_num(rh_npy_ctx[np.triu_indices_from(rh_npy_ctx, k=1)].flatten())
...
def plot_scatter(aw_values, fc_values, correlation, subject_id, avg=False):
    ...
    # Save the plot
    plot_filename = f'{"average_" if avg else "subject_"}{subject_id}_scatter_nosub_right.png'
...
output_file = "mean_spearman_correlations_aw40-rs_single_h_nosub.txt"
